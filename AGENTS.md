...
- Web tests run with `mockReset: true`: put a mock's default in the `vi.fn(impl)` argument or in `beforeEach`, never chained at construction or at module scope, since the reset runs before every test.
