...
def get_user_from_context(event: Dict[str, Any]) -> str:
    ...
    try:
        ...
        if not user_id:
            logger.warning("User ID not found in request context", extra={
                "request_context": request_context
            })
            raise AuthenticationError("User identity not found in request")
        
        logger.debug("Extracted user ID from context", extra={
            "user_id": user_id
        })
        ...
        logger.error("Failed to extract user from context", extra={
            "error": str(e),
...
def verify_user_owns_vault(user_id: str, vault_id: str, item_vault_id: str) -> None:
    ...
    if vault_id != item_vault_id:
        logger.warning("Vault ownership verification failed", extra={
            "user_id": user_id,
...
def verify_user_owns_resource(user_id: str, resource_user_id: str) -> None:
    ...
    if user_id != resource_user_id:
        logger.warning("Resource ownership verification failed", extra={
            "user_id": user_id,
...
def validate_cognito_token_claims(claims: Dict[str, Any]) -> bool:
    ...
    for claim in required_claims:
        if claim not in claims:
            logger.warning("Missing required claim in token", extra={
                "missing_claim": claim
            })
            return False
