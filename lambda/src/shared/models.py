...
class InitiateUploadRequest(BaseModel):
    ...
    vault_id: str = Field(
        ...,
        description="Vault ID for the file"
    )
    encrypted_tags: Optional[List[bytes]] = Field(
        default=None,
        description="List of encrypted tags"
    )
...
class InitiateUploadResponse(BaseModel):
    ...
    upload_id: Optional[str] = Field(
        default=None,
        description="Multipart upload ID (for large files)"
    )
...
class CreateVaultRequest(BaseModel):
    ...
    vault_salt: bytes = Field(
        ...
        min_length=16,
        max_length=16,
        description="16-byte vault salt for key derivation"
    )
...
class CreateShareRequest(BaseModel):
    ...
    is_password_protected: bool = Field(
        default=False,
        description="Whether share requires password"
    )
...
class GenerateRecoveryCodesResponse(BaseModel):
    ...
    recovery_codes: List[str] = Field(
        ...,
        description="List of recovery codes (displayed once)"
    )
...
class DynamoDBFileItem(BaseModel):
    ...
    GSI1PK: Optional[str] = Field(
        default=None,
        description="GSI1 partition key for tag queries"
    )
    GSI1SK: Optional[str] = Field(
        default=None,
        description="GSI1 sort key for tag queries"
    )
...
class DynamoDBShareItem(BaseModel):
    ...
    is_password_protected: bool = Field(
        default=False,
        description="Password protection flag"
    )
...
class DynamoDBRecoveryCodeItem(BaseModel):
    ...
    used_at: Optional[int] = Field(
        default=None,
        description="Usage timestamp (Unix epoch)"
    )
